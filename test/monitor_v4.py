...
def draw_animation(d_q):
    ...
    while True:
        ...
        if len(r_data) == 3 and r_data[0] == 'S':
            ...
            v = float(s_msg[usv_num][3])                               # 航速，为1时，当前栅格为巡航栅格，0<v<1时，遍历栅格
            if v == 1:   # 巡航路径航点
            ...
            # 接受速度为0的信息，并保存usv终止的列表，当列表中所有usv等于集群列表中所有的usv时，设置终止程序flag
            elif v == 0:  # usv集群任务结束
                end_usv_list.append(usv_num)
            else:         # usv遍历路径航点
        ...
        if show_animation:  # pragma: no cover
            ...
            if time.time() - tic > 1800:
                plt.savefig(str(k) + 'usv_swarm.png', dpi=600)
                time.sleep(0.1)

            plt.pause(dt)
            # TODO 判断终止程序flag，若激活，则保存图像，并终止本程序
            if len(end_usv_list) == total_usv_num:
                plt.savefig('usv_swarm.png', dpi=600)
                time.sleep(0.1)
                break
