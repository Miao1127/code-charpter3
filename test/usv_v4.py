...
def solver(rs_q, sd_q, rd_q, sp_q, rp_q, p_q):
    ...
    while thread_flag:
        ...
        # ********************测试代码逻辑用*********************************
        # u2z_d = read_csv.run('u2z_usv1.csv')
        # usv_path = read_nested_dict_json.run('usv1_path.json')
        # ******************************************************************
        ...
        while True:  # 当前usv发送的距离信息得到其他所有usv的回应后才停止发送
            ...
            if (u2z_mat[1:, 0] != 0).all():  # 集齐其他usv距离信息后，为保证让其他usv收集到当前usv的距离信息，需要额外发送若干次
                for i in range(10):          # 额外重复发送次数需要为队列的2倍以上，避免其他usv接收队列接收满后清空
                    sd_q.put(d_send_msg)
                break
        time.sleep(5)
        ...
        for key in tuple(usv_plan['u']):  # 更新占据分区列表
        ...
        # 5.3 将本次分配的分区编号添加到占据分区内，并判断是否还存在未分配的分区，若无，则以遍历路径终点为起点，出发点为终点，规划返航路径
        if len(occupied_zone) == len(v_list):  # 不存在还未分配的分区
            ...
            recovery = a_star.AStar(map_data, recovery_start, recovery_goal)
            recovery.run()
        ...
        # 判断是否所有的分区都被分配，若是，则向actor发送分配完毕信号，并停止本线程循环
        if len(occupied_zone) == len(v_list):
            thread_flag = False
            p_q.put(np.array([[0, 0, 0]]))  # 向actor传递分区全部分配完毕的信息，只传递一个数据，且第三个速度元素为0
            print('所有分区已被分配...')
...
def actor(ss_q, p_q):
    ...
    :return:
    """
    occupied_flag = 1  # 用于标记是否还存在可分配分区，1表示存在，0表示所有分区已被分配，不存在未被分配的分区
    total_path = p_q.get()
    while True:
        if not p_q.empty():
            add_path = p_q.get()
        ...
        if len(total_path) > 2 :
            ...
            v = total_path[0][2]  # 速度，1时为栅格为巡航模式，小于大于0时栅格为遍历模式，0为当前usv完成集群任务
            mutex.acquire()
            usv_info[usv_num][1] = len(total_path)               # 更新当前usv剩余路径长度
            usv_info[usv_num][3] = v                             # 更新遍历速度
            usv_info[usv_num][4] = x                             # 更新当前usv位置行编号
            usv_info[usv_num][5] = y                             # 更新当前usv位置列编号
            mutex.release()
            state_data = 'S' + '$' + str(usv_info) + '$' + '\n'  # 当前usv状态信息
            ss_q.put(state_data)
        # 从solvor接收一个所有分区都已被分配的信号，通过判断剩余路径长度和此信号来终止程序，终止前需要向监控端发送终止信号
        # 终止信号可以设定为速度0
        elif len(total_path) <= 2 and occupied_flag == 0:
            end_info = {usv_num: [0, 0, 0, 0, 0, 0]}             # usv路径达到终点信息
            state_data = 'S' + '$' + str(end_info) + '$' + '\n'  # 当前usv状态信息
            ss_q.put(state_data)
            ss_q.put(state_data)
            break
...
if __name__ == '__main__':
    ...
    q3 = Queue(10)       # 用于放置当前usv与各个分区的距离，并发送出去
    ...
    usv_num = '##1'                                  # 当前usv编号
    start_point = [12, 6]                            # 当前usv出发点
    cruise_speed = 1                                 # 巡航速度
    cover_speed = 0.5                                # 遍历速度
