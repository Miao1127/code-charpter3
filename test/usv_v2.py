    ...
    while True:
        ...
        while True:  # 当前usv发送的距离信息得到其他所有usv的回应后才停止发送
            ...
            if (u2z_mat[1:, 0] != 0).all():  # 集齐其他usv距离信息后，为保证让其他usv收集到当前usv的距离信息，需要额外发送若干次
                for i in range(5):          # 额外重复发送次数需要为队列的2倍以上，避免其他usv接收队列接收满后清空
                    sd_q.put(d_send_msg)
                    time.sleep(1)
                break
        ...
        sd_q.queue.clear()  # 清空发送当前usv与各个分区距离队列中的信息， 为下次发送新的信息做准备
        rd_q.queue.clear()  # 清空接收其他usv与各个分区距离的队列中的信息，为下次接收信息做准备
        time.sleep(2)
        ...
        sp_q.queue.clear()  # 清空存放决策结果的队列
        while True:
            s_plan = 'P' + '$' + str(usv_plan) + '$' + '\n'  # 当前usv决策信息
            sp_q.put(s_plan)
            time.sleep(1)
            if not rp_q.empty():
                other_plan = rp_q.get()  # 用于存储其他usv发送的决策方案
...
if __name__ == '__main__':
    ...
    q2 = LifoQueue()    # 用于接收其他usv发送的最新状态信息，实现集群协同
    q3 = Queue()        # 用于放置当前usv与各个分区的距离，并发送出去
    q4 = LifoQueue()     # 用于接收其他usv与各个分区的距离
    q5 = Queue()        # 用于放置当前usv的BPSO决策结果，并发送出去
    q6 = LifoQueue()     # 采用后进先出队列，用于接收其他usv的BPSO决策结果
