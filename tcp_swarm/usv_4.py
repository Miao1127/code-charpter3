...
import copy
import random
from file_operator import read_json, read_csv,  read_nested_dict_json
...
def solver(rs_q, rd_q, rp_q, p_q):
    ...
    while True:
        ...
        if k == 1:
            # ********************测试代码逻辑用*********************************
            u2z_d = read_csv.run('u2z_usv4.csv')
            usv_path = read_nested_dict_json.run('usv4_path.json')
            # ******************************************************************
        else:
            u2z_d, usv_path = point2zone.run(usv_start_info, occupied_zone, vertex_dict, map_data)  # 计算usv从当前位置到各个分区的距离
...
def multi_threading():
    ...
    t3 = threading.Thread(target=solver, args=(q2, q4, q6, q7))    # 当前usv求解器
    t4 = threading.Thread(target=actor, args=(q7,))                     # 当前usv运动模型线程
