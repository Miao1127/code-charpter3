...
def draw_animation(d_q):
    ...
    color_dict = {'##1': 'm', '##2': 'y', '##3': 'g', '##4': 'b', '##5': 'c', '##6': 'r', '##7': 'purple', '##8': 'gold',
                  '##9': 'lime'}  # 各个usv轨迹颜色设置
                ...
                for key in usv_dict_cruise:
                    ...
                    for i in usv_dict_cruise[key]:
                        if len(usv_dict_cruise[key][i]) >= 2:
                            plt.plot(usv_dict_cruise[key][i][1:, 0], usv_dict_cruise[key][i][1:, 1], color=color_dict[key],
                                     linestyle='dotted', linewidth=2.5)
                for key in usv_dict_cover:
                    # 绘制遍历轨迹
                    for i in usv_dict_cover[key]:
                        if len(usv_dict_cover[key][i]) >= 2:
                            plt.plot(usv_dict_cover[key][i][1:, 0], usv_dict_cover[key][i][1:, 1], color=color_dict[key],
                                     linestyle='solid', linewidth=1.5)
                for key in usv_dict:
                    plt.scatter(usv_dict[key][0][0], usv_dict[key][0][1], s=50, marker='*', c='k')  # 绘制各个usv起始位置
                    plt.scatter(usv_position[key][0], usv_position[key][1], s=50, marker='.', c='r')  # 绘制各个usv当前位置
